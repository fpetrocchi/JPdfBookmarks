= JPdfBookmarks
Flaviano Petrocchi; life888888 (Maven line)
:doctype: article
:encoding: utf-8
:lang: zh
:toc: left
...
== Project home and repository policy

* *Canonical source repository (current development):* https://github.com/fpetrocchi/JPdfBookmarks[fpetrocchi/JPdfBookmarks on GitHub]
* *Official Git mirror on SourceForge:* after Git is enabled on the project, use the clone URL shown there (pattern `https://git.code.sf.net/p/jpdfbookmarks/<repo-label>/`). It tracks the same history as GitHub; setup and push steps are in link:RELEASING.md[RELEASING.md] (sections *SourceForge Git mirror* and *SourceForge Files*).
* *Historical project home and downloads:* https://sourceforge.net/projects/jpdfbookmarks/[jpdfbookmarks on SourceForge]
* *Lineage:* This program is the same JPdfBookmarks originally developed in Subversion on SourceForge (GPL-3.0 since the 2010 open-source release). The Maven-based tree descends from SourceForge `branches/pdfbox` work (e.g. packaged as `jpdfbookmarks-code-r213-branches-pdfbox`) and incorporates the community fork https://github.com/life888888/JPdfBookmarks[life888888/JPdfBookmarks] (CJK GUI fixes, tooling). See link:AUTHORS[], link:THANKS[], link:CHANGELOG.md[], link:LICENSING.md[], and link:RELEASING.md[].
* *License:* GNU General Public License v3.0 — full text in link:COPYING[]; summary in link:LICENSE[].
...
== Optional JPEG2000 support (user-installed)

JPdfBookmarks does not bundle a JPEG2000 decoder in the official distribution.
If you need to open PDFs containing JPEG2000 images, you can add decoder jars
locally in your own installation.

=== Required jars

Download these two jars from Maven Central:
...
1. Close JPdfBookmarks.
2. Open your extracted JPdfBookmarks folder.
3. Copy both jars into the `lib/` directory (the same folder with other runtime jars).
4. Start JPdfBookmarks again (`jpdfbookmarks.exe`, `jpdfbookmarks_cli.exe`, or launcher script).

=== Verify

Open a PDF that previously logged:
`Cannot read JPEG2000 image: Java Advanced Imaging (JAI) Image I/O Tools are not installed`.
If the jars are loaded correctly, that error should disappear.

=== License note

The JPEG2000 plugin above includes code under the JJ2000 license, which upstream
documents as not GPL-3 compatible for project redistribution. For this reason,
JPdfBookmarks does not ship it by default. Installing it manually is an end-user choice.
