...
點擊　`jpdfbookmarks.exe` 或是 `jpdfbookmarks_cli.exe`，如果跳出對話窗顯示需要 Java Runtime Environment **21**（或類似版本提示），表示你沒有安裝 JRE 21 或 JDK 21。


[IMPORTANT]
.下載 JRE/JDK
====
鑒於 Oracle 的 JRE/JDK 授權爭議問題，我將 jpdfbookmarks.exe / jpdfbookmarks_cli.exe 提示安裝 JRE/JDK 的下載網址改為 https://adoptium.net/temurin/releases/?version=21
...
點擊下載的 Temurin 21 **JRE .msi**（檔名類似 `OpenJDK21U-jre_x64_windows_hotspot_...msi`）
出現視窗後 , 點選 **[Next]**
...
Q: 為何你已經安裝了非 Oracle 的 JRE/JDK，執行 jpdfbookmarks.exe / jpdfbookmarks_cli.exe 還是會跳出對話窗顯示需要 **Java Runtime Environment 21**（或類似版本）？難道只能裝 Oracle 的 JRE 嗎？
...
注意: 安裝 JRE 或 JDK 的版本須為 **21** 或更高（建議 Temurin 21 LTS）。
...
OpenJDK Runtime Environment Temurin-21... (build 21.0.x+...)
OpenJDK 64-Bit Server VM Temurin-21... (build 21.0.x+..., mixed mode, sharing)
----
...
== （選用）手動加入 JPEG2000 支援（最終使用者）

JPdfBookmarks 官方發佈包預設不包含 JPEG2000 解碼器。
如果你需要開啟含 JPEG2000 圖片的 PDF，可以在你自己的安裝目錄手動加入對應 jar。

=== 需要的 jar

請從 Maven Central 下載以下兩個檔案：
...
1. 關閉 JPdfBookmarks。
2. 開啟你解壓縮後的 JPdfBookmarks 安裝目錄。
3. 將上述兩個 jar 複製到 `lib/` 目錄（與其他執行期 jar 同一層）。
4. 重新啟動 JPdfBookmarks（`jpdfbookmarks.exe`、`jpdfbookmarks_cli.exe` 或 Linux 啟動腳本）。

=== 驗證方式

重新開啟先前失敗的 PDF。若設定成功，terminal 不應再出現：
`Cannot read JPEG2000 image: Java Advanced Imaging (JAI) Image I/O Tools are not installed`

=== 授權提醒

`jai-imageio-jpeg2000` 含有 JJ2000 授權內容，上游專案文件指出它不屬於 GPL-3 相容授權。
因此 JPdfBookmarks 不會預設隨程式打包此元件；是否安裝由最終使用者自行決定並承擔其授權條款。
